  ...
  try {
    switch (action) {
      ...
      case 'debugHeaders':
        ...
        break;
      case 'registrarCadastroCliente':
        result = actionRegistrarCadastroCliente(body);
        break;
      default:
...
function actionRegistrarCadastroCliente(body) {
  const user = requireAuth(body);
  if (!user) return { ok: false, error: 'Sessão inválida. Faça login novamente.' };
  ...
  // O envio por e-mail (com anexos) é feito pelo próprio vendedor, pelo app de e-mail dele.
  try {
    ...
    sheet.appendRow([
      ...
      d.telFinanceiro || '', d.tabelaPreco || '', d.observacao || ''
    ]);
  } catch (err) {
    // segue mesmo se o registro na planilha falhar; o importante é o vendedor conseguir enviar o e-mail
  }
